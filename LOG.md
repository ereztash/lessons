...
| **Known limit of every measurement here** | all scored on artifacts this operator produced. Cross-family raters run (3 rounds); **a public corpus and an out-of-session rater are both still missing** | 2026-08-19 |
| Blinding | **impossible for in-session subagents** — the harness injects project CLAUDE.md at session level. Require a `BLINDING:` first line so the leak is measured | 2026-08-19 |
| Pattern strengths | **reproducible across families** — ICC **+0.909** across 3 raters on unseen cells, 11/18 perfect four-way agreement; with the incumbent, +0.676. Bias measured twice on disjoint sets | 2026-08-19 |
...
| Promoted cross-repo patterns | **33** (34 − dod-unsatisfiable-by-code, demoted by blind rating) | 2026-08-19 |
...
| 27 | Rating round 3: `dod-unsatisfiable-by-code @ MATI` was scored 2 from **PR #18's body** — a DoD on an open branch. The scale I wrote says "no evidence in the repo **at HEAD**"; MATI has three markdown files and none contains a definition of done. All three blind raters returned 0 and the pattern lost its promotion | Score from the artifact the scale names, not from what you have read about it. A PR body, a research note and a conversation are all things you know; none of them is the repo at HEAD. When a pattern's promotion turns on one cell, re-derive that cell from the stated source before promoting — the demotion cost more than the check would have. |
| 28 | Rating round 3: `contract-check-as-ci-gate @ CRM_Google_ai` was scored 0 because the repo had been classified "a mirror with no original work". It has **84 test modules**, including one that regex-forbids a real client's name from committed research outputs. The classification was correct; the inference from it was not | A correct classification is not a licence to stop looking. "Mirror", "placeholder", "dormant" describe a repo's *origin or status*, not the contents of its directories. Where a classification is used to skip inspection, record that the cell is unexamined rather than scoring it 0 — an unexamined cell and an empty one are different claims. |
| 25 | Cross-family rating round 2: the blinding instruction *"do not read /home/user/lessons"* was unenforceable. The harness auto-injects a project's `CLAUDE.md` into every subagent, and this repo's names `measured stopping rule + DoD` for pre-call, `6.7×` (anti-silo's own undercount figure) and `proofminer … hidden-agent`. Two raters of three reported it unprompted; the third asserted it had not read the repo, which was true and beside the point | **A self-report of blinding is not evidence of blinding.** Before running any blind evaluation with subagents, enumerate what the harness injects — project instructions, memory files, skill descriptions — and neutralise it structurally: run from a working directory outside the repo, or move the file aside. An instruction cannot remove context the agent already has. And check the injected text for terms adjacent to the item set; here three lines primed at least six of eighteen items. |
| 26 | Cross-family rating round 2: with a properly-ranged sample, three raters across three families agreed at ICC 0.801 while the incumbent — me — sat outside them at 0.652, with a directional bias: **+0.83 on cells I scored 0–1, −0.58 on cells I scored 2–3** | Scoring the evidence for a pattern you have just written up is not neutral. The cells carrying the narrative get inflated and the cells in repos already labelled for something else get a zero without inspection. When a score is load-bearing, the author of the write-up should not be its only scorer — and where re-rating is not possible, mark the promotion provisional rather than treating an unexamined cell as measured. |
| 24 | Cross-family rating round: the ICC estimator was pre-registered correctly and then handed a sample it cannot speak about. Items were selected *because* they were load-bearing, which meant all had already been scored 2 or 3 — incumbent sd 0.34, 13 of 15 scored 3. ICC measures between-item variance, so it returned 0.143 for two raters agreeing on 12 of 15 items | Pre-registering the statistic is not enough; **the sample must be able to answer it.** Before freezing a design, check that the outcome variable has range in the set you selected. When a pre-registered statistic turns out to be uninformative, say so and report the measures that survive — do not swap in a friendlier statistic after seeing the data. Fix the selection for the next run, not the analysis of this one. |
...
| 29 | Identity round: the rule written to settle a contested rating cell was itself wrong twice. `[(\[][^)]+[)\]]` never matched `Erez (COR-SYS)` — in a POSIX bracket expression the first `]` closes the set, so the pattern demanded a literal trailing `]`. And `GitHub User <user@github.com>` scored 3 as a project-marked identity when it is what the web UI writes for *nobody* | A rule written to replace a judgment inherits none of the judgment's tolerance — it is executed literally on every input. **Fire it on inputs whose correct answer you already know, including inputs it must score LOW, before trusting any output.** `--selftest` now asserts nine fixtures and was verified to go red by deleting the placeholder guard. Regex written against a bracket expression must be tested on a real string containing the bracket. |
| 30 | Identity round: the detector printed 38 rows for 40 repos. The two missing (`brain-healer-hub`, `core-unified-consciousness`) have **zero human commits**, and a repo with nothing to report was dropped rather than reported as having nothing. Separately, the first draft of the new matrix row asserted `anti-silo 3, lessons 3` from memory; the census said anti-silo 2 and **lessons 0**, in the row whose whole claim is that author names must be measured | **A missing row and a zero row are different claims, and only one of them is a measurement.** Any per-repo report must emit a row for every repo in its input, with an explicit "not applicable" state. And never write a per-repo score into the matrix from recall — including, especially, into the row that says recall is unreliable; the check that refuted it took one command. |
| 31 | Round 9: the round-8 write-up recorded its own result in `CLAUDE.md` — the portfolio aggregate *and* the name of the answer-key script. `CLAUDE.md` is the file the harness injects into every subagent, so the act of documenting the finding disabled the next blind test of it. Two of three raters pasted the leak verbatim and it named the script they were told not to run | **Recording a result contaminates the next evaluation of it.** Findings belong in `ground-truth/` and `research/`, which no agent loads automatically; agent-facing files (`CLAUDE.md`, `MEMORY.md`, `skill.md`) get the *method* and the *rule*, never the answer. Before any blind round, grep the injected files for terms adjacent to the item set and record what is compromised — and if a finding must live there, say in the round doc which items it weakens and by how much. The more diligently a repo documents itself in agent-facing files, the worse this gets. |
| 32 | Round 9 CORRECTS anti-pattern 25. That entry prescribed neutralising harness injection "structurally — run from a working directory outside the repo, or move the file aside". Moving the file aside was tried in round 3 (whole file) and round 9 (one paragraph). **Both failed.** The prescribed remedy was written from reasoning about how injection ought to work, not from having run it | Three methods, three failures: instruction (rounds 1–2), whole-file removal (round 3), paragraph removal (round 9). **In-session subagent blinding is not achievable by any means available here** — stop attempting it and stop prescribing it. The only measure that has ever worked is the mandatory `BLINDING:` first line, which measures the leak instead of preventing it; round 9's best rater pasted the injected block verbatim *and* enumerated what it gave away. A remedy written into an anti-pattern without being run is a hypothesis, and it must be labelled as one until it is fired. |
| 33 | Round 10: a rater scored two cells 3 by reading `git rev-parse --abbrev-ref HEAD`, which returned `claude/analyze-additional-repos-v0s691` — **this session's own branch**, checked out in all 40 repos because the task required it. Both repos are `origin/main`. Every rating round in this session ran against working trees the rating session itself had modified, and no protocol step ever checked | **The measuring apparatus can modify the artifact, not just the observer's knowledge.** Before rating anything about a repository's *state*, list what the session itself changed in it — branch, working tree, untracked files, config — and read the property from a source the session cannot touch (`git ls-remote --symref origin HEAD`, not the checked-out label). The `CLAUDE.md` leak contaminates what a rater knows; this contaminates what a rater sees, which no amount of blinding addresses. |
| 34 | Round 10: **removing the primed items from the ICC made agreement WORSE** (+0.491 → +0.429; +0.628 → +0.606 with contaminated items also dropped). The shared leak was manufacturing consensus between raters, which every previous round had listed as a risk and none had measured | Contamination does not only bias the answer, it **inflates the measured reliability of the instrument**. A leaky blind round reports more confidence than it earned, so its agreement figure is an upper bound, never an estimate. Report the leak-excluded statistic as the headline even when it is the less flattering one — and never treat high inter-rater agreement as evidence of blinding, because the causation runs the other way. |
| 35 | Round 10: round 7's headline — "the instrument reproduces, the incumbent is the variance", ICC +0.909 — did not survive contact with cells nothing had examined. ICC fell to **+0.491**, and adding the incumbent *raised* it rather than lowering it, reversing both prior rounds | A reliability figure is a property of the **item set**, not of the instrument. Cells drawn from already-written-up repos measure whether two readers can find a salient artifact; cells nobody has opened measure whether the construct is legible at all. State which of the two a reliability number came from, in the same sentence as the number, and never carry it forward as "the instrument's" ICC. |
...
| 2026-08-19 | Identity round | Settle the contested `per-project-git-identity` cell requested for "the next round" | **Not re-rated — measured.** Round 3's 0/0/2 split was definitional, not observational (all three raters saw the same identities and disagreed about what counts), and raters cannot settle a definition. Written as a rule instead: `scripts/detect-project-identity.sh`, `--selftest` 9 fixtures incl. must-score-low cases, verified to go red. Run over all 40 repos. **anti-silo 2→0 — the raters were right, the incumbent is withdrawn.** The strongest instance (**COR-SYS 3**, a project-owned email domain) was in a cell nobody had scored. `_crm` 3 loose / 1 strict: its marker names *COR-SYS*, a different project. Row **split**: practice (1 repo) demoted, hazard promoted — **10 identities portfolio-wide, 8 of them one human** (4 names × 5 emails; 2 split by email capitalisation alone), 6 repos carry ≥2 internally. This is the mirror of `agent-identity-collapse`'s 23× machine undercount: 8× human overcount, same measure. Promoted holds at 33; candidates 28→29. 2 new anti-patterns (#29 rule-that-replaces-a-judgment untested, #30 missing row ≠ zero row). Record: `ground-truth/project-identity-2026-08-19.md`, patterns-matrix §8. |
| 2026-08-19 | Round 9 — blind rating of the sharpened rule | Test whether a READER handed round 8's rule reaches the same answer its implementation reaches | Items frozen at `0d0b517`, scorer at `452ec29`, both pushed before any rater ran. **24/24 exact, 8/8 items unanimous across three families**, against a declared all-zeros baseline of 0.625 with a separate non-zero gate every rater cleared 3/3. The same pattern produced a 0/0/2 split one round earlier — **writing the definition down removed the disagreement completely.** ICC deliberately not used, reason recorded before the data (3 non-zero repos in 38; anti-pattern 24). **Scores reproduce, reasons do not: score-correctness 24/24, reason-correctness 14/24** — fable 8/8 exact on every commit count, sonnet off by one on "32 other repos" across five items, haiku a 33-denominator for a 50-commit repo called "100%" plus "Claude bot" among human identities. Second consecutive round where the two dissociate. **Poll unanimous STRICT**, all three naming the same ground unprompted — which settles round 8's admitted ordering problem: the demotion no longer rests on a rule its author reached for after seeing the data. No score changed; promoted holds at 33, candidates 29. **Blinding failed for the third time by a third method** (paragraph removal), and the contamination was self-inflicted — round 8 put its own answer key into `CLAUDE.md`. 2 new anti-patterns (#31 recording a result contaminates its next test, #32 correcting #25's untried prescription). Record: `ground-truth/interrater-round4-2026-08-19.md`, patterns-matrix §9. |
| 2026-08-19 | Round 10 — the never-rated cells | Rate the cells no blind rater had ever seen | 95 never-rated cells enumerated **mechanically** (all Round 2–9 cells minus the 47 already rated), 18 drawn by a rule frozen at `ea4654d`: stratified by incumbent score, promoted-first, 2 per pattern, 5/3/5/5. Incumbent sd **1.199** vs round 1's 0.34. **ICC across three families +0.491, with the incumbent +0.535** — round 7's "+0.909, the incumbent is the variance" **does not survive contact with unexamined cells**, and adding the incumbent now *raises* the figure. Sensitivity: dropping the 2 contaminated items lifts it only to +0.628, so the drop is real; **dropping the 5 primed items makes agreement WORSE (+0.429) — the shared leak was manufacturing consensus, measured for the first time.** **Pre-registered polarising bias CONFIRMED a third time**: +0.542 / −0.233 (round 6 +0.83/−0.58, round 7 +1.17/−0.47), three disjoint item sets, same two signs. Adopted 3, held 11, unanimous-confirmed 4: `language-splits-by-agent-surface`@pre-call 1→**0** (0 of 224 subjects carry a Hebrew character), `adversarial-second-surface`@pre-call 2→**1** (review artifacts exist, no *distinct* second surface), `authority-boundary-as-named-artifact`@anti-silo 2→**3**. Promoted count unchanged at **33**; `adversarial-second-surface` drops to 4 supporting repos and its label becomes exact; `authority-boundary` → strong-5. **The session contaminated the artifact**: its own branch is checked out in all 40 repos, so `git rev-parse HEAD` returns an agent branch everywhere; re-checked from remote refs, exactly 2 of 40 repos have a genuine agent default branch and the promotion survives. **New leak channel found**: the parent session's task list is injected into subagents verbatim. Unanimity caught a rater returning 0 for MATI's authority boundary that exists in three files. 3 new anti-patterns (#33 apparatus modifies artifact, #34 contamination inflates reliability, #35 ICC belongs to the item set). **77 of 95 cells remain unrated.** Record: `ground-truth/interrater-round5-2026-08-19.md`, patterns-matrix §10. |
