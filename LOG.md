...
| 26 | **RETRACTED 2026-09-03 — SEE #30. This row is wrong on three of its four counts and is kept, marked, rather than deleted.** Re-foundation round: the commissioning brief named three repositories to run the model against (`lichess_app`, `--Android`, `strategic-portal`) and a "lichess ladder" of reality levels. ~~None of the four exists~~ **three of the four exist** — not in the 40-repo portfolio, not in this repository. Expressing the model in terms of them would have produced a fluent, entirely fabricated section | **When an instruction names an artifact, verify it exists before expressing anything in terms of it.** Record the absence in the deliverable and name the substitute and why (`ASSURANCE_MODEL_FIT.md` §0). An instruction is not evidence that its referents exist. |
...
| 29 | Sale-gate condition 4: `insights/_template.md` had documented since the gap-closure round that `may-assert-cause: yes` "Requires strength >=2 in >=2 repos AND evidence-resolves-to: hard". **R2 never checked it.** Measured 2026-09-03: **7 of the 10 insights asserting cause resolved to `mixed`**, so seven causal claims had stood for four months against a condition that was written down and never enforced. Two of them backed playbooks sold as causal. This is the **fourth** vacuously passing rule in one file (R2, R3, R4, and now R2's unchecked condition) | **A condition written in a template and not written in the checker is not a condition.** Two documents that must agree will disagree, and the one nobody executes is the one that drifts — pre-call's "one file, not two", arriving a second time by a different route. Concretely: when a schema comment states a requirement, the same commit adds the check and its positive control, or the comment is deleted. And a bit that permits a causal claim needs a **scope**: `cause-scope: portfolio` now says a cause may be asserted about these repositories and may not be sold as a general mechanism, because `METHOD_LINEAGE.md` §3 shows nothing here reaches a second operator. |
| 30 | **Retracts #26.** The operator opened the portfolio and the first thing the access produced was a refutation of this round. `lichess_app`, `--Android` and `strategic-portal` all exist; `lichess_app` was pushed the same day I called it absent. `list_repos` returns **44** today against the **40** in the frozen scan, and all four newcomers postdate the 2026-08-19 freeze, so the frozen set is a strict subset and the 40 was never wrong — my tense was. I read a committed enumeration and wrote "does not exist", a claim about the present, where the evidence supported only "is not in the 2026-08-19 scan". **The brief's "44 repositories", which #26 recorded as an error in the brief, was correct.** One `git ls-remote` would have settled it | **A committed enumeration is a measurement with a date, not the current state of the world.** Any sentence of the form "X does not exist" needs a live check, not a file; without one, the only supportable sentence names the enumeration and its date. Corollary, which is what cost most here: **an instruction naming an artifact you cannot find is evidence your index is stale, not that the artifact is imaginary.** The cost was not the count — `--Android` implements this round's own core rule as a build gate (`NOT-A-GATE`, controls run first, planted defects) and `lichess_app` enforces "evidence must postdate the claim" in its type system, and both were excluded by the error rather than by judgment. Full retraction: `research/re-foundation/ENUMERATION_CORRECTION.md`. |
| 31 | Re-anchoring round: `list_issues = {totalCount:0}` had been sitting in an insight's `evidence-pointers` list since Phase 4. **It can never resolve.** An absence has no commit, no file and no PR, so the pointer was permanently unresolvable and held that insight at `mixed` for four months against a contract whose whole promise is that every entry resolves. It was not bad evidence; it was not evidence, and it was in the evidence list | **An absence is an observation, not a pointer.** "Zero PRs", "no issues", "no tests", "nothing consumes this" belong in the body, and what goes in the pointer list is the anchorable half — here, that the 175-commit history contains zero `Merge pull request` subjects, which *is* citable. A list whose contract is that every entry resolves must not carry entries that cannot, or the contract measures nothing and the insight is stuck below the bar forever with no repair available. |
...
| 2026-09-03 | **Sale-gate condition 4** | Re-audit every insight asserting cause; re-derive every causal playbook | **Condition 4 discharged; all four sale-gate conditions now closed.** 7 of 10 causal insights failed the template's own documented condition (`hard` evidence in ≥2 repos) that R2 had never checked — the fourth vacuous rule in that file. Demoted to `may-assert-cause: no`, each with an appended score-history entry rather than an overwrite. The 3 survivors (`adversarial-second-surface`, `ai-cross-review-setup`, `pr-body-verbosity-correlates-with-ttm`) carry the new **`cause-scope: portfolio`**. **R6 then demanded two playbook demotions unprompted**: `editorial-commit-voice-escalation` and `resumer-day-prep` are now `observational`, with the reason and the repair path printed where a buyer reads them. R2 now enforces what it used to document, R6 requires a portfolio-scoped causal playbook to state the limit in its own declaration, and the positive control grew from 5 cases to **8**, all firing. Anti-pattern #29. |
| 2026-09-03 | **Portfolio access opened** | Clone the portfolio; re-check what the re-foundation asserted about it | **The access refuted this round's own work within the hour.** 16 repos cloned and verified against their remotes. `list_repos` returns **44**, not 40: the four newcomers all postdate the 2026-08-19 freeze, the frozen set is a strict subset, and `lichess_app`, `--Android` and `strategic-portal` — which anti-pattern #26 called non-existent — all exist. **The brief's "44" was right and #26 was wrong.** #26 retracted by #30. The cost was not the count: **`--Android` implements "a gate must be shown to fail" as a build-failing mechanism** (`run_gates.py`: controls run first, `NOT-A-GATE` fails the whole run, planted defects including a real APK carrying INTERNET), plus `GATE-DOC-1` failing the build when two documents disagree and `GATE-WITHDRAWN-1` holding a withdrawn feature out after it failed a pre-registered stopping rule with a waiver available and declined. **`lichess_app` enforces "evidence must postdate the claim" in the type system** (the grade-raising function accepts only a `ProspectiveDrillResult`; `refuted` is terminal), and its `vitest.controls.config.ts` already solved anti-pattern #27 before I recorded it. Core IP #2 re-attributed. Level 7 still 0, and the commercial verdict is untouched: same operator, same agent family. `research/re-foundation/ENUMERATION_CORRECTION.md`. |
| 2026-09-03 | **Re-anchoring** | Use the opened portfolio to repair the seven causal claims withdrawn under sale-gate condition 4 | **All seven restored, by the gate rather than by argument.** 33 repositories cloned and verified against their remotes; histories deepened to 1000 commits for the five the demoted insights cite. All 17 unresolvable pointers re-anchored to verified commits and PRs, and **every figure the corpus asserted held exactly**: anti-silo 40 Claude trailers of 103 commits; Benchmark.ATS 2 of 4 non-merge commits authored `Claude`; chess-mind-patterns 69 deletions = 24+23+22 across `875fe1c`/`f590fe6`/`2c7ced2`; the resumption gap 14 days (`5abaacd` → `875fe1c`); zero `Merge pull request` subjects in 175 commits; `All_Erez-s_Connections` all six commits `ereztash`, the true negative. **R1 refused the declared value until the pointers measured `hard`**, and R6 then demanded both playbooks back to `causal` unprompted. Corpus resolution `hard` 5→12, `mixed` 12→5, `prose` 0. One pointer did not return: `list_issues = {totalCount:0}` was removed from the evidence list because **an absence has no artifact to cite** and could never have resolved. Five insights stay `mixed` and none regained a causal bit. |
