...
| **ROUND 10 — THE NEVER-RATED CELLS, AND THE READING INVERTS AGAIN (2026-08-19).** 95 cells enumerated mechanically, 18 drawn stratified (incumbent sd **1.199** vs round 1's 0.34). **ICC +0.491 across families, +0.535 with me** — round 7's "+0.909, the instrument reproduces, the incumbent is the variance" **does not hold on cells nothing had examined**, and adding me now *raises* it | `ground-truth/interrater-round5-2026-08-19.md` |
| **THE LEAK WAS MANUFACTURING AGREEMENT — measured, not feared.** Dropping the 5 primed items makes ICC **worse** (+0.491→+0.429). Contamination inflates the *reliability* of the instrument, not just the answer. Every prior round listed this as a risk; none measured it. Headline must be the leak-excluded figure | round5 §1.1, LOG #34 |
| **POLARISING BIAS CONFIRMED A THIRD TIME** (pre-registered, direction only): **+0.542 / −0.233**, after +0.83/−0.58 and +1.17/−0.47. Three disjoint item sets, same two signs. It is a stable property of the incumbent | round5 §2 |
| **THE SESSION CONTAMINATED THE ARTIFACT IT WAS RATING.** `claude/analyze-additional-repos-v0s691` is checked out in all 40 repos, so `git rev-parse HEAD` returns an agent branch everywhere. Three raters split by how far they verified: label→3, saw `main` exists→1, `git ls-remote --symref origin HEAD`→**0** (correct). Re-checked from remote refs: exactly **2 of 40** repos have a genuine agent default branch (Agent-Architect, keepath) — **promotion survives, now on remote refs** | round5 §3, LOG #33 |
| **NEW LEAK CHANNEL**: the parent session's **task list** is injected into subagents verbatim ("Deep-dive _crm", "Re-test 17 promoted patterns", "Design the ground-truth rubric"). Every blinding audit enumerated `CLAUDE.md` and memory files and stopped there | round5 §4 |
| **Round 10 changes**: `language-splits-by-agent-surface`@pre-call 1→**0** (0 of 224 subjects have a Hebrew char) · `adversarial-second-surface`@pre-call 2→**1** (no *distinct* second surface) · `authority-boundary`@anti-silo 2→**3**. Promoted **33** unchanged. **77 of 95 cells still unrated; 11 of the 18 sampled held with raters split** | patterns-matrix §10 |
