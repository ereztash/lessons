...
| **`per-project-git-identity` AT RISK** — anti-silo split 0/0/2; if it is 0 the pattern has one repo and fails | round3 §4 |
...
| **Round 4 patterns**: 4 promoted (`self-refuting-kill-test`, `claim-downgrade-ledger`, `result-reported-with-its-own-ceiling`, `per-project-git-identity`). Total promoted **33**. Deep-dived repos: **12** | patterns-matrix §4 |
